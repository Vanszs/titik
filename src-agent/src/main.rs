...
//! | Invocation | Action |
//! |---|---|
//! | `titik` (no args) | **default** — ensure a daemon is running (spawn a detached daemon if none is up), then attach as a thin client. NO fallback to local. |
//! | `titik agents` | open the session hub (alias for `--resume`). |
//! | `titik --resume` | open the session hub. |
//! | `titik alone` | standalone no-daemon TUI ([`app::run`]); REFUSES if a daemon is already alive. The escape hatch (alias for `--local`). |
//! | `titik daemon <status\|kill\|restart\|clean>` | daemon management CLI then exit. |
//! | `titik --internet-fullmode-install [--force]` | provision Python full-mode (browser) env then exit. |
//! | `titik --internet-fullmode-uninstall` | remove Python full-mode env then exit. |
//!
...
//! | Flag | Action |
//! |---|---|
//! | `--local` | fronted by `titik alone`: force the OLD standalone local TUI; REFUSES if a daemon is already alive. |
//! | `--daemon` | run the headless titik-daemon event loop (no TUI). The default path execs this to spawn the daemon. |
...
fn main() -> anyhow::Result<()> {
    // Migrate legacy config dir (~/.simple-coder -> ~/.titik) before anything
    ...
    // --- short-circuit: `titik daemon <verb>` management CLI (no TUI) ---
    ...
    // --- short-circuit: `titik update` — stop daemon + run installer (no TUI) ---
    if opts.update {
        return match crate::app::run_update() {
            Ok(()) => Ok(()),
            Err(e) => {
                eprintln!("titik update failed: {e}");
                std::process::exit(1);
            }
        };
    }
    ...
    // --- headless path: run the titik-daemon event loop (no TUI) ---
    ...
    // Connects to ~/.titik/daemon.sock, renders the daemon's foreground session from
    ...
    // way `titik` launched before the daemon-by-default flip (Stage 7). GUARDED: if a
    ...
    // corrupts the locks. Direct the user to attach (plain `titik`) or kill the daemon
    // first, and exit non-zero. With no daemon up, run the standalone TUI normally.
    if opts.local {
        if app::daemon_alive() {
            eprintln!(
                "error: a titik daemon is running; use `titik` to attach to it, \
                 or `titik daemon kill` first (refusing to run a standalone local TUI \
    ...
    // This is THE daemon-by-default launch: `titik` with no flags first guarantees a
    // daemon is accepting (connect if one is up, else spawn a DETACHED `titik --daemon`
    ...
    // forwarded to the daemon. So a first-ever `titik` (no prior creds) reaches a usable
    // KeyInput screen through the client, not a crash.
    if let Err(e) = app::ensure_daemon_running(opts.resume) {
        eprintln!("error: could not start the titik daemon: {e:#} — try `titik --local`");
        std::process::exit(1);
    }
    app::client_run(opts)
}
